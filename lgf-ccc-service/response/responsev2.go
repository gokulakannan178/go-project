...
type ResponseSructbool struct {
	Response Responsebool `json:"response,omitempty"`
}
...
func With200V2bool(w http.ResponseWriter, msg string, data map[string]bool, platform string) {
	response := new(ResponseSructbool)
	response.Response.StatusCode = 200
	response.Response.Message = msg
	response.Response.Data = data
	dataB, err := json.Marshal(response)
	if err != nil {
		w.WriteHeader(422)
		fmt.Fprintf(w, "Invalid Data - "+err.Error())
		return
	}
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
