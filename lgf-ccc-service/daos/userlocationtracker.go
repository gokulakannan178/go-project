...
func (d *Daos) FilterUserLocationTracker(ctx *models.Context, userLocationTrackerFilter *models.FilterUserLocationTracker, pagination *models.Pagination) ([]models.RefUserLocationTracker, error) {
	...
	if userLocationTrackerFilter != nil {
		...
		if len(userLocationTrackerFilter.UserName) > 0 {
			query = append(query, bson.M{"userName": bson.M{"$in": userLocationTrackerFilter.UserName}})
		}
		if len(userLocationTrackerFilter.UserType) > 0 {
			query = append(query, bson.M{"userType": bson.M{"$in": userLocationTrackerFilter.UserType}})
		}
		//Regex
