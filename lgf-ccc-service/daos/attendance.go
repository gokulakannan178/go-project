...
func (d *Daos) GetTodayEmployeeTimeOff(ctx *models.Context) ([]models.TodayTimeoff, error) {
	...
	//lookup
	mainPipeline = append(mainPipeline, d.CommonLookup(constants.COLLECTIONUSER, "employeeId", "uniqueId", "employeeId", "employeeId")...)
...
func (d *Daos) EmployeeDayWiseAttendanceReport(ctx *models.Context, filter *models.DayWiseAttendanceReportFilter, pagination *models.Pagination) ([]models.EmployeeDayWiseAttendanceReport, error) {
	...
	if filter != nil {
		...
		query = append(query, bson.M{"status": bson.M{"$nin": []string{constants.EMPLOYEESTATUSDELETED, constants.EMPLOYEESTATUSREJECT, constants.EMPLOYEESTATUSRELIEVE, constants.EMPLOYEESTATUSONBORADING}}})
	...
	if pagination != nil {
		...
		countcursor, err := ctx.DB.Collection(constants.COLLECTIONUSER).Aggregate(ctx.CTX, paginationPipeline, nil)
		if err != nil {
