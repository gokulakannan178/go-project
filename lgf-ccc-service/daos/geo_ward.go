...
func (d *Daos) GetSingleWard(ctx *models.Context, code string) (*models.RefWard, error) {
	...
	mainPipeline = append(mainPipeline, d.CommonLookup(constants.COLLECTIONVILLAGE, "villageCode", "code", "ref.village", "ref.village")...)
...
func (d *Daos) FilterWard(ctx *models.Context, wardfilter *models.WardFilter, pagination *models.Pagination) ([]models.RefWard, error) {
	...
	if wardfilter != nil {
		...
		if len(wardfilter.CircleCode) > 0 {
			query = append(query, bson.M{"circleCode": bson.M{"$in": wardfilter.CircleCode}})
		}
		if len(wardfilter.VillageCode) > 0 {
			query = append(query, bson.M{"villageCode": bson.M{"$in": wardfilter.VillageCode}})
		}
	}
