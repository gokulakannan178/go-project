...
func (route *Route) UserRoutes(r *mux.Router) {
	...
	r.Handle("/user/registration/generateotp", Adapt(http.HandlerFunc(route.Handler.UserRegistrationGenerateOTP))).Methods("POST")
	r.Handle("/user/registration/validateotp", Adapt(http.HandlerFunc(route.Handler.UserRegistrationValidateOTP))).Methods("POST")
	r.Handle("/user/mobileno", Adapt(http.HandlerFunc(route.Handler.GetSingleUserWithMobileNumber))).Methods("GET")

	//Area Assign
	r.Handle("/projectmanager/assign", Adapt(http.HandlerFunc(route.Handler.AreaAssignForPM))).Methods("PUT")
...
func (route *Route) ConsumerRoutes(r *mux.Router) {
	...
	r.Handle("/citizen/registration/generateotp", Adapt(http.HandlerFunc(route.Handler.CitizenregistrationGenerateOTP))).Methods("POST")
	r.Handle("/citizen/registration/validateotp", Adapt(http.HandlerFunc(route.Handler.CitizenregistrationValidateOTP))).Methods("POST")
