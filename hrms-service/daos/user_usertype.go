...
func (d *Daos) SaveUserType(ctx *models.Context, UserType *models.UserType) error {
	res, err := ctx.DB.Collection(constants.COLLECTIONUSERTYPE).InsertOne(ctx.CTX, UserType)
	if err != nil {
		return err
	}
	UserType.ID = res.InsertedID.(primitive.ObjectID)
	return nil
}
...
func (d *Daos) GetSingleUserType(ctx *models.Context, UniqueID string) (*models.RefUserType, error) {
	...
	//Aggregation
	cursor, err := ctx.DB.Collection(constants.COLLECTIONUSERTYPE).Aggregate(ctx.CTX, mainPipeline, nil)
	if err != nil {
		return nil, err
	}
...
func (d *Daos) UpdateUserType(ctx *models.Context, UserType *models.UserType) error {
	...
	_, err := ctx.DB.Collection(constants.COLLECTIONUSERTYPE).UpdateOne(ctx.CTX, selector, updateInterface)
	if err != nil {
...
func (d *Daos) FilterUserType(ctx *models.Context, userTypefilter *models.UserTypeFilter, pagination *models.Pagination) ([]models.RefUserType, error) {
	...
	if pagination != nil {
		...
		//Getting Total count
		totalCount, err := ctx.DB.Collection(constants.COLLECTIONUSERTYPE).CountDocuments(ctx.CTX, func() bson.M {
			if query != nil {
				if len(query) > 0 {
...
func (d *Daos) EnableUserType(ctx *models.Context, UniqueID string) error {
	...
	_, err := ctx.DB.Collection(constants.COLLECTIONUSERTYPE).UpdateOne(ctx.CTX, query, update)
	if err != nil {
