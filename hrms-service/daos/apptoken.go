...
func (d *Daos) GetRegTokenWithParticulars(ctx *models.Context, ID string) (*models.Apptoken, error) {
	mainPipeline := []bson.M{}
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"status": "Active", "userid": ID}})
...
func (d *Daos) GetSingleApptokenWithUserID(ctx *models.Context, uniqueID string) (*models.RefApptoken, error) {
	...
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"userid": uniqueID}})
	// lookup
	//mainPipeline = append(mainPipeline, d.CommonLookup(constants.COLLECTIONPKGCATEGORY, "categoryId", "uniqueId", "ref.category", "ref.category")...)
