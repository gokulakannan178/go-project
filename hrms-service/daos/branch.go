...
// GetSingleBranchWithActiveaname : ""
func (d *Daos) GetSingleBranchActiveWithName(ctx *models.Context, uniqueID string) (*models.RefBranch, error) {
	mainPipeline := []bson.M{}
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"name": uniqueID, "status": constants.ONBOARDINGPOLICYSTATUSACTIVE}})
	//LookUp
	mainPipeline = append(mainPipeline, d.CommonLookup(constants.COLLECTIONORGANISATION, "organisationId", "uniqueId", "ref.organisationId", "ref.organisationId")...)
	d.Shared.BsonToJSONPrintTag("Branch query =>", mainPipeline)
	//Aggregation
	cursor, err := ctx.DB.Collection(constants.COLLECTIONBRANCH).Aggregate(ctx.CTX, mainPipeline, nil)
	if err != nil {
		return nil, err
