...
func (s *Service) SaveAttendanceWithEditEmployee(ctx *models.Context, attandeace *models.Attendance) error {
	...
	attandeace.CaseLOP = "Nil"
	attandeace.LoginMode = "Edit"
	attandeace.PayRoll = constants.ATTENDANCESTATUSPAID
	var res1 []string
	if attandeace.TotalWorkingHoursStr != "" {
		...
		attandeace.WorkingHoursMins = WorkingHoursMins
	}
...
func (s *Service) ClockinAttendance(ctx *models.Context, att *models.Attendance) error {
	...
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		...
		Employee, err := s.Daos.GetSingleEmployee(ctx, att.EmployeeId)
		if err != nil {
			return err
		}
		// Attendancecsheck, err := s.Daos.GetSingleAttendanceByEmployeeIdAndStateValue(ctx, uniqueIdcurrentformat, att.EmployeeId, constants.ATTENDANCESTATUSLOGIN)
		// if err != nil {
		// 	fmt.Println(err)
		// 	return err
		// }
		// if Attendancecsheck != nil {
		// 	return errors.New("Attendance already Founded")
		// }
		var workschedulehouse float64
		if Employee != nil {

			fmt.Println("Employee Workschedule===>", Employee.WorkScheduleID)
			WorkSchedule, err := s.Daos.GetSingleWorkSchedule(ctx, Employee.WorkScheduleID)
			if err != nil {
				return err
			}
			if WorkSchedule == nil {
				return errors.New("Please update employee Workschedule")
			}
			workschedulehouse = WorkSchedule.WorkingHoursinDay * 60
...
func (s *Service) ClockoutAttendance(ctx *models.Context, att *models.ClockoutAttendance) error {
	...
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		...
		//	p1 := Attendance.PunchIn
		//fmt.Println(p1)
		//	p2 := Attendance.PunchOut
		//fmt.Println(p2)
		...
		if Attendance.PayRoll != constants.ATTENDANCESTATUSHOLIDAY {
			switch days {
			case "Sunday":
				if WorkSchedule.Sunday != true {
					...
					mins := WorkSchedule.WorkingHoursinDay * 60
					percentage := (Attendance.TotalWorkingMins / mins) * 100

					fmt.Println("percentage==>", percentage)

					if percentage >= WorkSchedule.FullPay {
					...
					if percentage <= WorkSchedule.FullPay {
						Attendance.PayRoll = constants.ATTENDANCESTATUSPARTIALPAY
						Attendance.CaseLOP = "Nil"

					}
					if percentage <= WorkSchedule.PartialPay {
						Attendance.PayRoll = constants.ATTENDANCESTATUSLOP
						Attendance.CaseLOP = constants.EMPLOYEELOPCASECALCULATE
			...
			fmt.Println("TotalWorkingMins==>", Attendance.TotalWorkingMins)
		...
		// AttendanceLog.State = constants.ATTENDANCESTATUSLOGOUT
		// AttendanceLog.Status = constants.ATTENDANCESTATUSACTIVE
		// AttendanceLog.Date = &currentdate
		// AttendanceLog.WorkingHours = diff.Minutes()
		// AttendanceLog.Notes = att.Notes
		// dberr = s.Daos.SaveAttendanceLog(ctx, AttendanceLog)
		// if dberr != nil {
		// 	return dberr
...
func (s *Service) EmployeeAttendanceTodayStatus(ctx *models.Context, EmployeeId string, UniqueID string) (*models.EmployeeAttendanceTodayStatus, error) {
	...
	fmt.Println("Employee.WorkScheduleID=====>", Employee.WorkScheduleID)
	fmt.Println("WorkSchedule=====>", WorkSchedule)
	...
	workschedulehouse := WorkSchedule.WorkingHoursinDay * 60
	if workschedulehouse == AttendanceLog.TotalWorkingHours {
		AttendanceLog.OverTime = 0
	}
	if workschedulehouse < AttendanceLog.TotalWorkingHours {
		AttendanceLog.OverTime = AttendanceLog.TotalWorkingHours - workschedulehouse
	}
	if workschedulehouse > AttendanceLog.TotalWorkingHours {
		AttendanceLog.OverTime = 0
	}
...
func (s *Service) DayWiseAttendanceReport(ctx *models.Context, filter *models.DayWiseAttendanceReportFilter) (att *models.DayWiseAttendanceReport, err error) {
	defer ctx.Session.EndSession(ctx.CTX)
	Employee, err := s.Daos.GetSingleEmployee(ctx, filter.EmployeeId)
	if err != nil {
		return nil, err
	}
	if Employee == nil {
	...
	if len(attendancereport.Days) > 0 {
		...
		for k, v := range attendancereport.Days {
			days := v.Date.Weekday().String()
			loc, _ := time.LoadLocation("Asia/Kolkata")
			if v.PunchIn != nil {
				PunchIn := v.PunchIn.In(loc)
				fmt.Println("PunchIn", PunchIn)
				attendancereport.Days[k].PunchIn = &PunchIn
			}
			if v.PunchOut != nil {
				PunchOut := v.PunchOut.In(loc)
				fmt.Println("PunchOut", PunchOut)
				attendancereport.Days[k].PunchOut = &PunchOut
			}
			switch days {
			case "Sunday":
				if WorkSchedule.Sunday == true {
					attendancereport.Days[k].PaidTime = WorkSchedule.WorkingHoursinDay
				} else {
					attendancereport.Days[k].PaidTime = 0
				}
			case "Monday":
				if WorkSchedule.Monday == true {
					attendancereport.Days[k].PaidTime = WorkSchedule.WorkingHoursinDay
		...
		attendancereport.WorkScheduleHours = WorkSchedule.WorkingHoursinDay
...
func (s *Service) AttendanceEmployeeStatistics(ctx *models.Context, EmployeeId string) (*models.AttendanceEmployeeStatistics, error) {
	...
	if WorkSchedule != nil {
		AttendanceLog.TodayTotalWorkingHours = WorkSchedule.WorkingHoursinDay
	}
...
func (s *Service) DailyAttendanceCorn() {
	c := context.TODO()
	...
	for k, v := range employees {
		Employee, err := s.Daos.GetSingleEmployee(ctx, v.UniqueID)
		if err != nil {
			log.Println(err)
		}
		if Employee.WorkScheduleID == "" {
			log.Println("pls update the Workschedule the EmployeeId-", v.Name)
		}
		if Employee.WorkScheduleID != "" {
			WorkSchedule, err := s.Daos.GetSingleWorkSchedule(ctx, Employee.WorkScheduleID)
			if err != nil {
			...
			Attendance.EmployeeId = v.UniqueID
			Attendance.Status = constants.ATTENDANCESTATUSPENDING
			days := time.Now().Weekday().String()
			if WorkSchedule != nil {
				...
				log.Panicln("Corn WorkSchedule missing--->", err)
			}
			t := time.Now()
			created := models.Created{}
			created.On = &t
			...
			log.Println("b4 Attendance.created")
			checkattendance, err := s.Daos.GetSingleAttendanceByEmployeeIdWithLoginMode(ctx, uniqueIdcurrentformat, employees[k].UniqueID, constants.ATTENDANCESTATUSTIMEOFF)
			if err != nil {
				log.Panicln("checkattendance--->", err)
			}

			if checkattendance != nil {
				fmt.Println("checkattendance==>", checkattendance.UniqueID, checkattendance.EmployeeId)
			}
			if checkattendance == nil {
				err = s.Daos.SaveAttendanceWithUpsertUnPalanned(ctx, Attendance)
				if err != nil {
...
func (s *Service) AllEmployeeAttendanceApprove(ctx *models.Context, Attendance *models.AllEmployeeAttendanceApprove) error {
	...
	defer ctx.Session.EndSession(ctx.CTX)
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		for _, v := range Attendance.Employees {
			for _, v2 := range v.UniqueID {
				dberr := s.Daos.EmployeeAttendanceApprove(ctx, v.EmployeeId, v2)
				if dberr != nil {
					return dberr
				}
			}
...
func (s *Service) AllEmployeeAttendanceReject(ctx *models.Context, Attendance *models.AllEmployeeAttendanceApprove) error {
	...
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		for _, v := range Attendance.Employees {
			for _, v2 := range v.UniqueID {
				dberr := s.Daos.EmployeeAttendanceReject(ctx, v.EmployeeId, v2)
				if dberr != nil {
					return dberr
				}
			}
...
func (s *Service) GetTodayEmployeeUplannedLeave(ctx *models.Context) ([]models.TodayTimeoff, error) {
	attendance, err := s.Daos.GetTodayEmployeeUplannedLeave(ctx)
	if err != nil {
		return nil, err
	}
	return attendance, nil
...
func (s *Service) GetTodayEmployeeAbsent(ctx *models.Context, filter *models.DayWiseAttendanceReportFilter) (*models.TodayAbsent, error) {
	TodayAbesent := new(models.TodayAbsent)
	err := s.TodayAbsentAccess(ctx, filter)
	if err != nil {
		return nil, err
	}
	AppeovedLeaves, err := s.Daos.GetTodayEmployeeApprovedTimeOff(ctx, filter)
	if err != nil {
		return nil, err
	}
	fmt.Println("No.Of AppeovedLeaves", len(AppeovedLeaves))

	PendingLeaves, err := s.Daos.GetTodayEmployeePendingTimeOff(ctx, filter)
	if err != nil {
		return nil, err
	}
	fmt.Println("No.Of PendingLeaves", len(PendingLeaves))
	TodayAbesent.ApprovedLeave = append(TodayAbesent.ApprovedLeave, AppeovedLeaves...)
	TodayAbesent.PendingLeaveRequest = append(TodayAbesent.PendingLeaveRequest, PendingLeaves...)
	return TodayAbesent, nil
}
func (s *Service) TodayAbsentAccess(ctx *models.Context, filter *models.DayWiseAttendanceReportFilter) (err error) {
	if filter != nil {
