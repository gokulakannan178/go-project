...
func (h *Handler) GetSingleDepartmentUniqueCheck(w http.ResponseWriter, r *http.Request) {
	...
	department, err := h.Service.GetSingleDepartmentUniqueCheck(ctx, UniqueID, orgid)
	if err != nil {
		if err.Error() == "mongo: no documents in result" {
			response.With500mV2(w, "failed no data for this id", platform)
			return
