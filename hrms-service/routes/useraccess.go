...
func (route *Route) UserAclRoutes(r *mux.Router) {
	...
	r.Handle("/useracl/usertype", Adapt(http.HandlerFunc(route.Handler.SaveUserAclWithUpsert))).Methods("POST")
	r.Handle("/useracl/usertype", Adapt(http.HandlerFunc(route.Handler.GetSingleUserAclWithUserType))).Methods("GET")
