...
func (s *Service) UpdatePropertyFYDemandLogPropertyID(ctx *models.Context, uniqueIds *models.UpdatePropertyUniqueID) error {
	...
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		for _, v := range uniqueIds.UniqueIDs {
			...
			err = s.Daos.UpdatePropertyFYDemandLogPropertyID(ctx, uniqueIds)
			if err != nil {
				return err
			}
		}
