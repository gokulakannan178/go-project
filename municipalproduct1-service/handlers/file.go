...
func (h *Handler) DocumentUpload(w http.ResponseWriter, r *http.Request) {
	...
	respDocStart := h.ConfigReader.GetString(h.Shared.GetCmdArg(constants.ENV) + "." + constants.DOCLOCD)

	fileuri := docStart + scenario + "/" + s + handler.Filename
	responseURI := respDocStart + scenario + "/" + s + handler.Filename
...
func (h *Handler) DocumentsUpload(w http.ResponseWriter, r *http.Request) {
	...
	for k, fh := range fhs {
		...
		fmt.Println("fileuri=", fileuri)
		responseURI := respDocStart + scenario + "/" + s + fhs[k].Filename
