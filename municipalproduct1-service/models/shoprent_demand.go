...
type ShopRentCalcQueryFilter struct {
...
func (demand *ShopRentDemand) CalcQuery(filter *ShopRentCalcQueryFilter) ([]bson.M, error) {
	...
	fyQuery = append(fyQuery, bson.M{
		"$lookup": bson.M{
			...
			"as":   "ref.shopRentTax",
			"let":  bson.M{"fyFrom": "$from", "fyTo": "$to", "shopCategoryId": "$$shopCategoryId", "shopSubCategoryId": "$$shopSubCategoryId", "id": "$uniqueId"},
			"pipeline": []bson.M{
	...
	mainPipeline = append(mainPipeline, bson.M{
		"$lookup": bson.M{
			"from":     "financialyears",
			"as":       "fy",
			"let":      bson.M{"shopCategoryId": "$shopCategoryId", "shopSubCategoryId": "$shopSubCategoryId", "calcDateFrom": "$dateFrom", "calcDateTo": "$dateTo"},
			"pipeline": fyQuery,
		},
	})
...
func (demand *ShopRentDemand) CalcDemand() error {
	demand.ShopRent.Demand = ShopRentTotalDemand{}
	ct := time.Now()
	if demand != nil {
		for k, _ := range demand.FY {
			//demand.FY[k].Details.Tax = demand.FY[k].Ref.ShopRentTax.Rate
			demand.FY[k].Details.Tax = demand.RefShopRent.RentAmount
...
func (demand *ShopRentDemand) CalcCollectionQuery() ([]bson.M, error) {
	var mainPipeline []bson.M
...
func (demand *ShopRentDemand) CalcCollection(mtps []RefShopRentPayments) error {
	for _, v := range mtps {
...
func (demand *ShopRentDemand) CalcPendingCollectionQuery() ([]bson.M, error) {
	var mainPipeline []bson.M
...
func (demand *ShopRentDemand) CalcPendingCollection(mtps []RefShopRentPayments) error {
	for _, v := range mtps {
...
func (demand *ShopRentDemand) CalcOutStanding() error {
...
type RefShopRentDemandFYLog struct {
	ShopRentDemandFYLog `bson:",inline"`
}

type ShopRentDemandFYLog struct {
