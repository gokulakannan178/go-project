...
func (demand *UserChargeMonthlyDemand) CalcQuery(filter *UserChargeMonthlyCalcQueryFilter) ([]bson.M, error) {
	var mainPipeline []bson.M
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"uniqueId": demand.RefPropertyUserCharge.UniqueID}})
	...
	if filter.OmitPayedMonths {
		mainPipeline = append(mainPipeline, bson.M{"$lookup": bson.M{
			"from": constants.COLLECTIONUSERCHARGEPAYMENTSFY,
			"as":   "payedYears",
			"let":  bson.M{"UserChargeId": "$uniqueId"},
			"pipeline": []bson.M{
				bson.M{"$match": bson.M{"$expr": bson.M{"$and": []bson.M{
					bson.M{"$eq": []string{"$UserChargeId", "$$UserChargeId"}},
	...
	// 	return bson.M{
	// 		"$lookup": bson.M{
	// 			"from": constants.COLLECTIONUSERCHARGEINDIVIDUALRATEMASTER,
	// 			"as":   "ref.UserChargeTaxV2",
	...
	// 					bson.M{"$eq": []string{"$UserChargeId", filter.UserChargeID}},
	// 				}}}},
	...
	// 	return bson.M{"$addFields": bson.M{"ref.UserChargeTaxV2": bson.M{"$arrayElemAt": []interface{}{"$ref.UserChargeTaxV2", 0}}}}
	// }
	...
	fyQuery = append(fyQuery, bson.M{
		...
		bson.M{"$addFields": bson.M{"ref.UserChargeTax": bson.M{"$arrayElemAt": []interface{}{"$ref.UserChargeTax", 0}}}})
...
// func (demand *UserChargeMonthlyDemand) CalcDemand() error {
// 	// ct := time.Now()
// 	if demand != nil {
// 		demand.UserCharge.Demand = UserChargeTotalDemand{}
// 		for k, v := range demand.FY {
// 			for k2, v2 := range v.Months {
// 				demand.FY[k].Months[k2].Details.Tax = demand.RefPropertyUserCharge.RentAmount
// 				if demand.IndividualRateMaster == "Yes" {
// 					demand.FY[k].Months[k2].Details.Tax = v2.Ref.UserChargeTaxV2.Rate
// 				}
...
// 	return nil
// }
// func (demand *UserChargeMonthlyDemand) CalcCollectionQuery() ([]bson.M, error) {
// 	var mainPipeline []bson.M
// 	//Fetch The Shop Rent Payments
// 	mainPipeline = append(mainPipeline, bson.M{
// 		"$match": bson.M{
// 			"UserChargeId": demand.UserCharge.UniqueID,
// 			"status":       constants.UserChargePAYMENTSTATUSCOMPLETED,
// 		}})

// 	return mainPipeline, nil
// }

// func (demand *UserChargeMonthlyDemand) CalcCollection(mtps []RefUserChargePayments) error {
// 	for _, v := range mtps {
...
// 	return nil
// }
// func (demand *UserChargeMonthlyDemand) CalcPendingCollectionQuery() ([]bson.M, error) {
// 	var mainPipeline []bson.M
// 	//Fetch The Mobile Tower Payment
// 	mainPipeline = append(mainPipeline, bson.M{
// 		"$match": bson.M{
// 			"UserChargeId": demand.UserCharge.UniqueID,
// 			"status":       constants.UserChargePAYMENTSTATUSPENDING,
// 		}})

// 	return mainPipeline, nil
// }

// func (demand *UserChargeMonthlyDemand) CalcPendingCollection(mtps []RefUserChargePayments) error {
// 	for _, v := range mtps {
...
// 	return nil
// }
// func (demand *UserChargeMonthlyDemand) CalcOutStanding() error {
...
type RefUserChargeMonthlyDemandFYLog struct {
	UserChargeMonthlyDemandFYLog `bson:",inline"`
}

type UserChargeMonthlyDemandFYLog struct {
	UserChargeMonthlyDemandLog `bson:",inline"`
	...
	Ref struct {
		UserChargeTax UserChargeRateMaster `json:"UserChargeTax,omitempty" bson:"UserChargeTax,omitempty"`
	} `json:"ref,omitempty" bson:"ref,omitempty"`
}

type UserChargeMonthlyDemandLog struct {
	...
	Months        []struct {
		...
		Ref struct {
			//	UserChargeTaxV2 RefUserChargeindividualRateMaster `json:"UserChargeTaxV2,omitempty" bson:"UserChargeTaxV2,omitempty"`
		} `json:"ref,omitempty" bson:"ref,omitempty"`
