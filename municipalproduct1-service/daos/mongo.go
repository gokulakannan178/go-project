...
func GetDB(ctx context.Context, daos *Daos) (*mongo.Database, mongo.Session, *mongo.Client) {
	...
	isTLS := daos.Config.GetString(daos.Shared.GetCmdArg(constants.ENV) + "." + constants.ISMONGOTLS)
	...
	if isTLS == "Yes" {
		tlsFileURL := daos.Config.GetString(daos.Shared.GetCmdArg(constants.ENV) + "." + constants.MONGOTLSFILE)
		fmt.Println("tls file location =", tlsFileURL)
		tlsConfig := &tls.Config{
			RootCAs: createCertPool(tlsFileURL),
		}
	...
	client, err := mongo.Connect(ctx, clientOptions.ApplyURI(mongodbURL))
	if err != nil {
		panic(err.Error())
	}
	...
	secReadPref := readpref.Secondary()
	dbOpts := options.Database().SetReadPreference(secReadPref)
	return client.Database(daos.mongoDB, dbOpts), sess, client
}

func createCertPool(paths ...string) *x509.CertPool {

	pool := x509.NewCertPool()
	for _, path := range paths {
		pool.AddCert(loadCert(path))
	}
	return pool
}
