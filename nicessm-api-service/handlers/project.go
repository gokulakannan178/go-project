...
func (h *Handler) ProjectUniquenessCheck(w http.ResponseWriter, r *http.Request) {
	platform := r.URL.Query().Get("platform")
	var ctx *models.Context
	var err error
	ctx = app.GetApp(r.Context(), h.Service.Daos)
	...
	_, err = h.Service.ProjectUniquenessCheck(ctx, org, projectname)
	if err != nil {
	...
	m := make(map[string]interface{})
	m["data"] = "project already founded"
	response.With201mV2(w, "project name already founded", platform)
}
