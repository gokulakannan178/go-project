...
func (h *Handler) OrganistationProjectDetails(w http.ResponseWriter, r *http.Request) {
	...
	if resType == "excel" {
		file, err := h.Service.OrganistationProjectDetailsExcelV2(ctx, filter)
		if err != nil {
			response.With500mV2(w, "failed - "+err.Error(), platform)
			return
		}
		w.Header().Set("Content-Type", "application/octet-stream")
		w.Header().Set("Content-Disposition", "attachment; filename=OrganisationProjectDetailsReport.xlsx")
		w.Header().Set("ocntent-Transfer-Encoding", "binary")
		file.Write(w)
		return
	}
	//	data:=
	data, err := h.Service.OrganistationProjectDetails(ctx, filter)
	if err != nil {
		response.With500mV2(w, "failed - "+err.Error(), platform)
		return
	}
