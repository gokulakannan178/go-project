...
func (h *Handler) DocumentUploadWithTxt(w http.ResponseWriter, r *http.Request) {
	platform := r.URL.Query().Get("platform")
	...
	s := fmt.Sprintf("%X", b)
	docStart := h.ConfigReader.GetString(h.Shared.GetCmdArg(constants.ENV) + "." + constants.DOCLOC)
	filesUri := strings.Split(handler.Filename, ".")
	handler.Filename = fmt.Sprintf("%v.txt", filesUri[0])
