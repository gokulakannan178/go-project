...
func (h *Handler) GetSingleStateWeatherDataWithCureentDate(w http.ResponseWriter, r *http.Request) {
	...
	stateweatherdata, err := h.Service.GetSingleStateWeatherDataWithCureentDate(ctx, UniqueID)
	if err != nil {
		if err.Error() == "mongo: no documents in result" {
			response.With500mV2(w, "failed no data for this id", platform)
			return
