...
func (route *Route) StateRoutes(r *mux.Router) {
	...
	r.Handle("/geo/report", Adapt(http.HandlerFunc(route.Handler.GeoDetatilsReport))).Methods("GET")
	r.Handle("/geo/upload", Adapt(http.HandlerFunc(route.Handler.GeoUploadExcelWithNames))).Methods("POST")
	r.Handle("/state/getweatheralert", Adapt(http.HandlerFunc(route.Handler.GetWeatherDataWithSeverityType))).Methods("POST")
}
...
func (route *Route) DistrictRoutes(r *mux.Router) {
	...
	r.Handle("/district/getweatheralert", Adapt(http.HandlerFunc(route.Handler.GetDistrictWeatherDataWithSeverityType))).Methods("POST")
...
func (route *Route) VillageRoutes(r *mux.Router) {
	...
	r.Handle("/addvillage/withgrampanchat", Adapt(http.HandlerFunc(route.Handler.SaveVillageWithGrampanchatBlock))).Methods("POST")
}
