...
func (d *Daos) SaveDistrictWeatherDataWithUpsert(ctx *models.Context, stateweatherdata *models.DistrictWeatherData) error {

	opts := options.Update().SetUpsert(true)
	updateQuery := bson.M{"district": stateweatherdata.District, "uniqueId": stateweatherdata.UniqueID}
	updateData := bson.M{"$set": stateweatherdata}
	_, err := ctx.DB.Collection(constants.COLLECTIONDISTRICWEATHERDATA).UpdateOne(ctx.CTX, updateQuery, updateData, opts)
	return err
}
...
func (d *Daos) FilterDistrictWeatherData(ctx *models.Context, districtweatherdatafilter *models.DistrictWeatherDataFilter, pagination *models.Pagination) ([]models.RefDistrictWeatherData, error) {
	...
	if districtweatherdatafilter != nil {
		...
		if districtweatherdatafilter.DateRange != nil {
			//var sd,ed time.districtweatherdatafilter
			if districtweatherdatafilter.DateRange.From != nil {
				sd := time.Date(districtweatherdatafilter.DateRange.From.Year(), districtweatherdatafilter.DateRange.From.Month(), districtweatherdatafilter.DateRange.From.Day(), 0, 0, 0, 0, districtweatherdatafilter.DateRange.From.Location())
				ed := time.Date(districtweatherdatafilter.DateRange.From.Year(), districtweatherdatafilter.DateRange.From.Month(), districtweatherdatafilter.DateRange.From.Day(), 23, 59, 59, 0, districtweatherdatafilter.DateRange.From.Location())
				if districtweatherdatafilter.DateRange.To != nil {
					ed = time.Date(districtweatherdatafilter.DateRange.To.Year(), districtweatherdatafilter.DateRange.To.Month(), districtweatherdatafilter.DateRange.To.Day(), 23, 59, 59, 0, districtweatherdatafilter.DateRange.To.Location())
				}
...
func (d *Daos) GetSingleDistrictWeatherDataWithCureentDate(ctx *models.Context, UniqueID string) (*models.RefDistrictWeatherData, error) {
	id, err := primitive.ObjectIDFromHex(UniqueID)
	if err != nil {
		return nil, err
	}
	t := time.Now()
	fmt.Println("district time==>", t)
	query := []bson.M{}
	query = append(query, bson.M{"district": id})
