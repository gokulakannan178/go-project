...
func (d *Daos) SaveUserAclWithUpsert(ctx *models.Context, useracl *models.UserAcl) error {
	...
	updateQuery := bson.M{"userName": useracl.UserName}
	//Aggregation
...
func (d *Daos) GetSingleUserAclWithUserName(ctx *models.Context, UniqueID string) (*models.RefUserAcl, error) {
	id, err := primitive.ObjectIDFromHex(UniqueID)
	if err != nil {
		return nil, err
	}
	mainPipeline := []bson.M{}
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"userName": id}})
	//Aggregation
	cursor, err := ctx.DB.Collection(constants.COLLECTIONUSERACL).Aggregate(ctx.CTX, mainPipeline, nil)
	if err != nil {
		return nil, err
