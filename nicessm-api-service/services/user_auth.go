...
func (s *Service) Login(ctx *models.Context, login *models.Login) (string, bool, error) {
	data, err := s.Daos.GetSingleUserWithUserName(ctx, login.UserName)
	if err != nil {
		fmt.Println(err)
	...
	if data.Status == constants.USERSTATUSINIT {
		return "", false, errors.New("awaiting activation please contact administrator")
	}
	if data.Status != constants.USERSTATUSACTIVE {
		return "", false, errors.New("please contact administrator")
	}
	if data.Status == constants.USERSTATUSDELETED {
...
func (s *Service) OTPLoginGenerateOTP(ctx *models.Context, login *models.Login) error {
	...
	//otp, _ := s.GenerateOTP(constants.USERLOGIN, login.UserName, constants.PHONEOTPLENGTH, constants.OTPEXPIRY)
	// if err != nil {
	// 	return errors.New("Otp Generate Error - " + err.Error())
	// }
	key := fmt.Sprintf("%v_%v", constants.USERLOGIN, login.UserName)
	var otp models.Otp
	otp.Otp = "9999"
	...
	err = s.SendSMSV2(ctx, login.UserName, text)
	if err != nil {
...
func (s *Service) OTPLoginValidateOTP(ctx *models.Context, login *models.OTPLogin) (*models.RefUser, bool, error) {
	...
	key := fmt.Sprintf("%v_%v", constants.USERLOGIN, login.Mobile)
	otp := new(models.Otp)
