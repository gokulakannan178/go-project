...
func (s *Service) SaveContentTranslation(ctx *models.Context, contenttranslator *models.ContentTranslation) error {
	...
	contenttranslator.Status = constants.CREATED
...
func (s *Service) ApprovedContentTranslation(ctx *models.Context, Content *models.ApprovedContentTranslation) error {
	if err := ctx.Session.StartTransaction(); err != nil {
		return err
	}
	defer ctx.Session.EndSession(ctx.CTX)
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		...
		err := s.Daos.ApprovedContentTranslation(ctx, Content)
		if err != nil {
			if err = ctx.Session.AbortTransaction(sc); err != nil {
				return errors.New("Transaction Aborted with error" + err.Error())
			}
...
func (s *Service) RejectedContentTranslation(ctx *models.Context, Content *models.RejectedContentTranslation) error {
	if err := ctx.Session.StartTransaction(); err != nil {
		return err
	}
	defer ctx.Session.EndSession(ctx.CTX)
	if err := mongo.WithSession(ctx.CTX, ctx.Session, func(sc mongo.SessionContext) error {
		...
		err := s.Daos.RejectedContentTranslation(ctx, Content)
		if err != nil {
			if err = ctx.Session.AbortTransaction(sc); err != nil {
				return errors.New("Transaction Aborted with error" + err.Error())
			}
