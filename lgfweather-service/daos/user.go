...
//UpdateUser : ""
func (d *Daos) UpdateUserWithCompanyId(ctx *models.Context, user *models.User) error {
	selector := bson.M{"comanyId": user.CompanyID}
...
//GetSingleUserWithMobileNo : ""
func (d *Daos) GetSingleUserWithMobileNoForFPO(ctx *models.Context, mobileno string) (*models.RefUser, error) {
	mainPipeline := []bson.M{}
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"mobile": mobileno, "type": "FPO"}})
	mainPipeline = append(mainPipeline, d.CommonLookup(constants.COLLECTIONFPO, "companyId", "uniqueId", "ref.fpo", "ref.fpo")...)
...
//GetSingleUserWithMobileNo : ""
func (d *Daos) ChkMobileNoFPO(ctx *models.Context, mobileno string) (*models.RefFPO, error) {
	mainPipeline := []bson.M{}
	mainPipeline = append(mainPipeline, bson.M{"$match": bson.M{"mobile": mobileno}})
	...
	var users []models.RefFPO
	var user *models.RefFPO
	if err = cursor.All(ctx.CTX, &users); err != nil {
		return nil, err
	}
	if len(users) > 0 {
