...
func (d *Daos) GetActiveBlock(ctx *models.Context) ([]models.Block, error) {
	mainPipeline := []bson.M{}
	query := []bson.M{}
	query = append(query, bson.M{"status": constants.BLOCKSTATUSACTIVE})
...
func (d *Daos) FilterBlockByName(ctx *models.Context, blockfilter *models.BlockFilter, pagination *models.Pagination) ([]models.BlockName, error) {
	mainPipeline := []bson.M{}
	query := []bson.M{}
	if blockfilter != nil {
		if len(blockfilter.StateName) > 0 {
			query = append(query, bson.M{"imdStateName": bson.M{"$in": blockfilter.StateName}})
		}
		if len(blockfilter.DistrictName) > 0 {
			query = append(query, bson.M{"imdDistrictName": bson.M{"$in": blockfilter.DistrictName}})
		}
	}
	//Adding $match from filter
	if len(query) > 0 {
	...
	mainPipeline = append(mainPipeline, bson.M{"$group": bson.M{
		...
		"blockName":    bson.M{"$push": "$imdBlockName"},
