...
func (s *Service) FPOLoginGenerateOTP(ctx *models.Context, login *models.Login) error {
	...
	// text := fmt.Sprintf("Dear %v, /n Otp For lgfweather FPO login is %v .", data.Name, otp)
	...
	fmt.Println(fpo)
	if fpo.Email != "" {
		err := s.SendEmail("lgfweather - OTP for FPO login", []string{fpo.Email}, text)
		if err != nil {
...
func (s *Service) FPORegistrationLoginGenerateOTP(ctx *models.Context, login *models.FPO) error {
	...
	//Send SMS
	// text := fmt.Sprintf("Dear user, Otp For lgfweather FPO registration is %v .", otp)
	msg := "Otp For lgfweather FPO registration is" + otp
	...
	fmt.Println(login)
	if login.Email != "" {
		err := s.SendEmail("lgfweather - OTP for FPO registration", []string{login.Email}, msg)
		if err != nil {
			fmt.Println("Sms Sending Error - " + err.Error())
		}
