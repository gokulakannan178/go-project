...
func (route *Route) UserAuthRoutes(r *mux.Router) {
	...
	r.Handle("/user/auth/fpo/otplogin/generateotp", Adapt(http.HandlerFunc(route.Handler.FPOLoginGenerateOTP))).Methods("POST")
	r.Handle("/user/auth/fpo/otplogin/validateotp", Adapt(http.HandlerFunc(route.Handler.FPOLoginValidateOTP))).Methods("POST")
	r.Handle("/user/auth/fporegistration/generateotp", Adapt(http.HandlerFunc(route.Handler.FPORegistrationLoginGenerateOTP))).Methods("POST")
	r.Handle("/user/auth/fporegistration/validateotp", Adapt(http.HandlerFunc(route.Handler.FPORegistrationLoginValidateOTP))).Methods("POST")
	r.Handle("/user/auth/fporegistration/mobile/uniqueness", Adapt(http.HandlerFunc(route.Handler.FPORegistrationMobileUniqueness))).Methods("POST")
}

// FPO/regiostration/mobile/uniqueness
