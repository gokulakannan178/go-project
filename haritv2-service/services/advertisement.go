...
func (s *Service) SaveAdvertisement(ctx *models.Context, advertisement *models.Advertisement) error {
	...
	advertisement.Status = constants.ADVERTISEMENTSTATUSACTIVE
	t := time.Now()
...
func (s *Service) GetSingleAdvertisement(ctx *models.Context, UniqueID string) (*models.RefAdvertisement, error) {
	popup, err := s.Daos.GetSingleAdvertisement(ctx, UniqueID)
	if err != nil {
		return nil, err
	}
	return popup, nil
...
//FilterAdvertisement :""
func (s *Service) FilterAdvertisement(ctx *models.Context, filter *models.AdvertisementFilter, pagination *models.Pagination) ([]models.RefAdvertisement, error) {
	defer ctx.Session.EndSession(ctx.CTX)
	return s.Daos.FilterAdvertisement(ctx, filter, pagination)
