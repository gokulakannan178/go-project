...
//OTPLoginGenerateOTP :
func (s *Service) FPOLoginGenerateOTP(ctx *models.Context, login *models.Login) error {
	fpo, err := s.Daos.ChkMobileNoFPO(ctx, login.UserName)
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	// if fpo.Status != constants.FPOSTATUSACTIVE {
	// 	return errors.New("FPO not Active Please Contact Administrator")
	// }
	//Send OTP
	if login.UserName == "0000001234" {
		otp = "9999"
	} else {
		otp, err = s.GenerateOTP(constants.USERLOGIN, login.UserName, constants.PHONEOTPLENGTH, constants.OTPEXPIRY)
		if err != nil {
	...
	// text := fmt.Sprintf("Dear %v, /n Otp For Harit FPO login is %v .", data.Name, otp)
	text := fmt.Sprintf(constants.FPOLOGINGENERATEOTP, fpo.ChairMan, otp)
	err = s.SendSMS(login.UserName, text)
	...
	fmt.Println(fpo)
	if fpo.Email != "" {
		err := s.SendEmail("Harit - OTP for FPO login", []string{fpo.Email}, text)
		if err != nil {
...
//OTPLoginValidateOTP :
func (s *Service) FPOLoginValidateOTP(ctx *models.Context, login *models.OTPLogin) (*models.RefFPO, bool, error) {

	fpo, err := s.Daos.ChkMobileNoFPO(ctx, login.Mobile)
	if err != nil {
		fmt.Println(err)
		return nil, false, err
	}
	...
	if fpo.Status == constants.USERSTATUSINIT {
		return nil, false, errors.New("Awaiting Activation")
	}
	if fpo.Status != constants.USERSTATUSACTIVE {
		return nil, false, errors.New("Please Contact Administrator")
	}
	if login.Mobile == "0000001234" {
		fpo.Ref.FPO = &fpo.FPO
		return fpo, true, nil
	} else {
...
//OTPLoginGenerateOTP :
func (s *Service) FPORegistrationLoginGenerateOTP(ctx *models.Context, login *models.FPO) error {
	data, err := s.Daos.ChkMobileNoFPO(ctx, login.Mobile)
	if err != nil {
		fmt.Println(err)
		return err
	}
	if data != nil {
		return errors.New("mobile no already available")
	}
	if err != nil {
		return err
	}
	...
	otp, err := s.GenerateOTP(constants.USERLOGIN, login.Mobile, constants.PHONEOTPLENGTH, constants.OTPEXPIRY)
	if err != nil {
		return errors.New("Otp Generate Error - " + err.Error())
	}
	//Send SMS
	// text := fmt.Sprintf("Dear user, Otp For Harit FPO registration is %v .", otp)
	msg := "Otp For Harit FPO registration is" + otp
	text := fmt.Sprintf(constants.COMMONTEMPLATE, "user", "URNCCH", "OTP for registration", msg, "http://urncc.org")
	...
	fmt.Println(login)
	if login.Email != "" {
		err := s.SendEmail("Harit - OTP for FPO registration", []string{login.Email}, msg)
		if err != nil {
			fmt.Println("Sms Sending Error - " + err.Error())
		}
...
//OTPLoginValidateOTP :
func (s *Service) FPORegistrationLoginValidateOTP(ctx *models.Context, login *models.OTPLogin) (*models.RefUser, bool, error) {

	data, err := s.Daos.GetSingleUserWithMobileNoForFPO(ctx, login.Mobile)
	if err != nil {
		fmt.Println(err)
		return nil, false, err
	}
	...
	// 	return nil, false, errors.New("User Type is invalid")
	// }
...
// FPORegistrationMobileUniqueness : ""
func (s *Service) FPORegistrationMobileUniqueness(ctx *models.Context, mobileNo string) error {

	res, err := s.Daos.ChkMobileNoFPO(ctx, mobileNo)
	if err != nil {
		return err
	}
	if res != nil {
